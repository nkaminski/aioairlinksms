...
import asyncio
import logging
from typing import Any, Coroutine, Callable, List, Optional, Tuple

from .exceptions import AirlinkConnectionClosedError, AirlinkSMSMessageDecodeError, AirlinkSMSMessageEncodeError
...
class AirlinkSMSMessage():
    ...
    def __init__(self, phone_number: str, message: str):
        if not phone_number.isnumeric():
            raise AirlinkSMSMessageDecodeError("Cannot create an airlink message object with a non-numeric phone number!")
    ...
    def elaborate(self) -> List[AirlinkSMSMessage]:
        ...
        """
        parts = [ self.message[i:i + self.AIRLINK_MESSAGE_MAX_LENGTH] for i in range(0, self.length, self.AIRLINK_MESSAGE_MAX_LENGTH) ]
    ...
    def serialize(self) -> bytes:
        ...
        if message_length > self.AIRLINK_MESSAGE_MAX_LENGTH:
            raise AirlinkSMSMessageEncodeError(f"Message of length {message_length} is too long to be sent as an SMS message!")
        
        # Represent the message as a string of UPPERCASE hex characters
        encoded_message = self.message.encode('ascii').hex().upper()

        # Form the message
        serialized_message = self.AIRLINK_MESSAGE_START + ','.join([self.phone_number, self.DATA_TYPE_ASCII, str(message_length), encoded_message]) + self.AIRLINK_MESSAGE_END
    ...
    def deserialize(cls, message_bytes: bytes) -> AirlinkSMSMessage:
        ...
        if decoded_message_length != length:
            raise AirlinkSMSMessageDecodeError("Decoded message length mismatch: expected {length}, got {decoded_message_length}")
...
class AirlinkSMSUDPServerProtocol(AirlinkSMSUDPClientProtocol):
    def __init__(self, reply_client: AirlinkSMSUDPClientProtocol,
                       on_message_received: Optional[Callable[[AirlinkSMSMessage, AirlinkSMSUDPClientProtocol], Coroutine[None, None, bool]]]=None,
...
async def create_message_handler(
    ...
    """
    logger.debug("Creating Airlink message handler, remote_addr=%s, remote_port=%s, local_bind_addr=%s, local_bind_port=%s",
                 remote_addr, remote_port, local_bind_addr, local_bind_port)
    ...
    client_transport, client_protocol = await loop.create_datagram_endpoint(
        lambda: AirlinkSMSUDPClientProtocol(),
    ...
    server_transport, server_protocol = await loop.create_datagram_endpoint(
        lambda: AirlinkSMSUDPServerProtocol(
            reply_client=client_protocol,
            on_message_received=on_message_received,
            on_connection_lost=server_done
        ),
